function imgVer(Config) {
    ...
    //随机读取图片库的图片
    var imgRandom = Math.floor(Math.random()*imgLibrary.length);//从图库随机抽取
    ...
    html += '<canvas id="puzzleBox" width="' + w + '" height="' + h + '" style="position:absolute;left:0;top:0;z-index:1;"></canvas>';
    html += '</div>';
    html += '<div class="puzzle-lost-box" style="position:absolute;width:' + w + 'px;height:' + h + 'px;top:0;left:' + left_Num + 'px;z-index:2;">';
    html += '<canvas id="puzzleShadow" width="' + w + '" height="' + h + '" style="position:absolute;left:0;top:0;z-index:4;"></canvas>';
    html += '<canvas id="puzzleLost" width="' + w + '" height="' + h + '" style="position:absolute;left:0;top:0;z-index:4;"></canvas>';
    html += '</div>';
    html += '<p class="ver-tips" style="z-index:5"></p>';
    html += '</div>';
    html += '<div class="re-btn" style="z-index:3"><a></a></div>';
    html += '</div>';
    html += '<br>';
    ...
    $('.re-btn a').unbind("click");
    $(document).off("click",".re-btn a").on("click",".re-btn a", function () {
        console.log("ss")
        imgVer(Config);
    })
}
